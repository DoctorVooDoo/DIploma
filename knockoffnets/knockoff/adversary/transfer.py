    ...
    def get_transferset(self, budget):
        ...
        with tqdm(total=budget) as pbar:
            for t, B in enumerate(range(start_B, end_B, self.batch_size)):
                remaining_budget = budget - len(self.transferset)
                sample_size = min(self.batch_size, remaining_budget, len(self.idx_set))
                idxs = np.random.choice(list(self.idx_set), replace=False, size=sample_size)
                ...
                resized_images = []
                for i in idxs:
                    img = self.queryset[i][0]
                    img = transforms.Resize((375, 500))(img) # Замените на желаемый размер
                    resized_images.append(img) 

                x_t = torch.stack(resized_images).to(self.blackbox.device)
